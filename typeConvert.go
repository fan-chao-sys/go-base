...
func main131() {
	...
	// 同时当使用 ParseUint 方法把字符串转换成数字时，或者使用 FormatUint 方法把数字转换成字符串时，都需要提供第二个参数 base，这个参数表示的是数字的进制，即标识字符串输出或输入的数字进制。
	ui64, err := strconv.ParseUint(str, 10, 32)
	fmt.Printf("字符串转换为uint64: %d \n", num)

	st2 := strconv.FormatUint(ui64, 2)
	...
	var ak Supplier = &DigitSupplier{value: 1}
	fmt.Println(ak.Get())

	bk, ok := (ak).(*DigitSupplier)
	fmt.Println(bk, ok)
