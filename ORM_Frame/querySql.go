...
func mainQuery() {
	...
	// Joins 一个衍生表
	// 可用联接来联接派生表。
	// SELECT `order`.`user_id`,`order`.`finished_at` FROM `order` join (SELECT MAX(order.finished_at) as latest FROM `order` left to join user on order.user_id = user.id WHERE user.age > 18 GROUP BY `order`.`user_id`) q on order.finished_at = q.latest
	...
	db.Model(&Order{}).Joins("join (?) q on order.finished_at = q.latest", query).Scan(&result)
